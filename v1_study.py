...
# Material (Neo-Hookean) — runtime-updatable
mu_0      = ti.field(dtype=ti.f32, shape=())       # shearing modulus
lambda_0  = ti.field(dtype=ti.f32, shape=())       # first Lamé parameter
...
y0, base_x = 0.325, 0.50         # mean base position
A, ω       = 0.025, 0.50        # vertical oscillation amplitude & angular freq
base_y     = y0
time_t     = 0.0
...
# ──────────────────────────────────────────────────────────────────────────────
@ti.func
def set_material(E, nu):
    mu_0[None]     = E / (2.0 * (1.0 + nu))
    lambda_0[None] = E * nu / ((1.0 + nu) * (1.0 - 2.0 * nu))

@ti.func
def neo_hookean_stress(F_i):
    J_det = F_i.determinant()
    FinvT = F_i.inverse().transpose()
    return mu_0[None] * (F_i - FinvT) + lambda_0[None] * ti.log(J_det) * FinvT
